...
function createProgramStateService({ db }) {
  ...
  const stmtLastRenders = db.prepare(`
    SELECT id, host_id, renders FROM program_runs
  ...
  function getLastRenders(programId, hostId) {
    ...
    try {
      const renders = JSON.parse(row.renders);
      return Array.isArray(renders) ? renders.map((item) => ({ runId: row.id, hostId: row.host_id, ...item })) : [];
    } catch { return []; }
  }
