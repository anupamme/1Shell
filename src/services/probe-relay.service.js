...
const RELAY_SYNC_TIMEOUT_MS = 15000;
const RELAY_SAMPLE_LOOKBACK_MS = 5 * 60_000;
const DEFAULT_RELAY_PORT = 3302;
...
function createProbeRelayService({ db, hostService, probeAgentService }) {
  ...
  function getUpstream(id, { includeToken = false } = {}) {
    const row = stmts ? stmts.getUpstream.get(id) : memory.upstreams.get(id);
    return row ? normalizeUpstream(row, { includeToken }) : null;
  }

  function buildServerUrlFromHost(host, portValue) {
    const port = Math.max(1, Math.min(parseInt(portValue, 10) || DEFAULT_RELAY_PORT, 65535));
  ...
  async function syncUpstream(row) {
    ...
    try {
      ...
      const minSinceMs = Date.now() - RELAY_SAMPLE_LOOKBACK_MS;
      const sinceMs = Math.max(lastSyncMs ? lastSyncMs - 60_000 : minSinceMs, minSinceMs);
      const since = `?since=${encodeURIComponent(new Date(sinceMs).toISOString())}`;
