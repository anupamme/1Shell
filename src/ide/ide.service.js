  ...
  async function handleMessage({ socket, sessionId, message, context, safeMode, claudeCodeEnabled, unlimitedTurns, entry }) {
    ...
    try {
      while (round < MAX_TOOL_ROUNDS) {
        round++;
        recordIdeAgentTurn(runId, round, {
          status: 'running',
          stage: 'decide',
          metadata: {
            entry: session.entry,
        ...
        recordIdeAgentTransition(runId, 'decide', 'IDE streaming loop requesting model decision.', { round });
        ...
        try {
          ...
          const compactedMessages = compactMessages(session.messages);
          const skillRoster = buildSkillRoster();
          const runtimeDecisionContext = buildAgentDecisionContext(agentRuntime.getState?.(runId) || null);
          const baseSystem = session.system + buildIdeAgentSystemDirective(session.agentTaskProfile) + skillRoster + runtimeDecisionContext;
          const apiBody = JSON.stringify({
            model,
            max_tokens: 8192,
          ...
          for (let attempt = 0; attempt <= MAX_PROVIDER_TRANSIENT_RETRIES; attempt++) {
            ...
            try {
              ...
              recordTraceEvent('provider', 'provider_request_started', {
                ...
                hostId: session.hostId,
                toolName: 'model_provider',
                summary: `streaming round=${round} attempt=${attempt}`,
                data: { round, attempt, timeoutMs: 240000 },
              });
              const resp = await fetch(proxyUrl, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
              ...
              if (!resp.ok) {
                ...
                recordTraceEvent('provider', 'provider_http_error', {
                  ...
                  toolName: 'model_provider',
                  summary: `provider returned HTTP ${resp.status}`,
                  data: { round, attempt, status: resp.status, bodyPreview: errText.substring(0, 500) },
                });
                throw new Error(`Provider 返回 ${resp.status}: ${errText.substring(0, 300)}`);
              }
              ...
              recordTraceEvent('provider', 'provider_headers_received', {
                ...
                hostId: session.hostId,
                toolName: 'model_provider',
                summary: `streaming round=${round} headers received`,
                data: { round, attempt, elapsedMs: Date.now() - fetchStartedAt },
              });
              data = await streamAnthropicSSE(resp.body, ac, session, socket, sessionId, runId);
              clearTimeout(overallTimeout);
              session.abortController = null;
              ...
              recordTraceEvent('provider', 'provider_stream_completed', {
                ...
                hostId: session.hostId,
                toolName: 'model_provider',
                summary: `streaming round=${round} completed`,
                data: {
                  round,
                  attempt,
                  elapsedMs: Date.now() - fetchStartedAt,
        ...
        const agentDecision = recordAgentControllerDecision({
          runtime: agentRuntime,
          runId,
          turn: round,
          turnIndex: round - 1,
        ...
        if (agentDecision.command.type !== 'act' && (toolCalls.length === 0 || data.stop_reason === 'end_turn')) {
          const finalText = textParts.join('');
          const finalization = evaluateAgentControllerFinalization({
            runtime: agentRuntime,
            runId,
            turn: round,
            turnIndex: round - 1,
            result: agentDecision.result || { text: finalText, status: data.stop_reason === 'end_turn' ? 'unknown' : '' },
            command: agentDecision.command,
            finalText,
            repairCount: session.finalizationRepairRounds || 0,
            additionalGates: [
              ({ state, finalText: candidateText, repairCount }) => evaluateIdeAgentFinalTurn({
                taskProfile: session.agentTaskProfile,
                state,
          ...
          const finalGate = finalization.finalGate || { reasons: [] };
          if (finalization.shouldContinue) {
            session.finalizationRepairRounds = Number(session.finalizationRepairRounds || 0) + 1;
            const shouldAutoVerify = finalGate.verifierPlan?.required === true
              && finalGate.verifierPlan?.ok !== true
              && ['missing'].includes(String(finalGate.verifierPlan?.status || ''));
            const verifierResult = shouldAutoVerify
              ? await runRuntimeVerifierPlanForIde('final_gate_requires_verification')
              : null;
            const feedback = verifierResult?.attempted
              ? formatIdeVerifierExecutorFeedback(verifierResult)
              : (finalization.message || finalGate.message || '');
            session.messages.push({ role: 'user', content: feedback });
            recordIdeAgentPhase(runId, 'context', `Runtime gate required another agent turn: ${(finalGate.reasons || []).join(', ')}`);
            recordTraceEvent('policy', 'final_turn_repaired', {
              source: 'ide',
              runId,
              sessionId,
              hostId: session.hostId,
              summary: (finalGate.reasons || []).join(', '),
              data: verifierResult?.attempted ? {
                verifierStatus: verifierResult.status || '',
                verifierReasons: verifierResult.reasons || [],
                verifierActions: verifierResult.actions || [],
              } : {},
            });
            if (isRunCurrent(session, runId)) emitToSession(session, socket, 'ide:thinking', { sessionId, runId });
            continue;
          }
          const finalResult = buildIdeFinalResult(runId, textParts, {
            sessionId,
            round,
            entry: session.entry,
            forcedTaskStatus: finalization.forcedStatus || finalGate.forcedStatus || '',
            finalGateReasons: finalGate.reasons || [],
            controllerAction: finalization.action || '',
          });
          if (isRunCurrent(session, runId)) {
            emitToSession(session, socket, 'ide:done', {
              sessionId,
        ...
        const runToolCall = async (tc) => {
          ...
          const agentPolicyResult = guardIdeAgentToolCall(runId, tc, session);
          if (agentPolicyResult) {
            ...
            emitToSession(session, socket, 'ide:tool-end', {
              ...
              toolUseId: tc.id,
              name: tc.name,
              result: toolContentPreview(agentPolicyResult.content).substring(0, 4000),
              is_error: true,
            });
            recordIdeAgentObservation(runId, {
              ...
              ok: false,
              isError: true,
              content: toolContentPreview(agentPolicyResult.content).substring(0, 4000),
              error: toolContentPreview(agentPolicyResult.content).substring(0, 1000),
            });
            return {
              type: 'tool_result',
              tool_use_id: tc.id,
              content: compactToolResultForModel(tc.name, agentPolicyResult.content),
              is_error: true,
            };
          }
          let ideToolStarted = false;
          const emitIdeToolStart = () => {
            if (ideToolStarted) return;
            ideToolStarted = true;
            emitToSession(session, socket, 'ide:tool-start', { sessionId, runId, toolUseId: tc.id, name: tc.name, input: tc.input });
          };
          const phaseId = tc.name === 'verify_outcome'
            ...
            : (tc.name === 'ask_user' || tc.name === 'request_secret' ? 'context' : 'act');
          recordIdeAgentPhase(runId, phaseId, `Calling tool ${tc.name}.`);
          const agentToolCall = (tc.name === 'ask_user' || tc.name === 'request_secret')
            ? startIdeAgentToolTrace(runId, tc, session)
            : null;
          ...
          if (tc.name === 'ask_user' || tc.name === 'request_secret') {
            ...
            endIdeAgentToolTrace(runId, agentToolCall, result);
            return {
              type: 'tool_result',
              tool_use_id: tc.id,
          ...
          if (policyResult) {
            ...
            recordIdeAgentObservation(runId, {
              ...
              ok: false,
              isError: true,
              content: toolContentPreview(result.content).substring(0, 4000),
              error: toolContentPreview(result.content).substring(0, 1000),
            });
            endIdeAgentToolTrace(runId, agentToolCall, result);
          ...
          const runtimeOwnsApproval = true;
          if (!runtimeOwnsApproval && shouldRequestAgentApproval(tc.name, {
            readonlyTools: READONLY_TOOLS,
            sideEffectTools: SIDE_EFFECT_TOOLS,
            controlTools: AGENT_CONTROL_TOOLS,
            input: tc.input || {},
          })) {
          ...
          const toolAc = new AbortController();
          const unregisterToolCancel = registerCancelHandler(session, () => toolAc.abort());
          const runtimeDispatchOptions = createIdeRuntimeDispatchOptions({
            socket,
            sessionId,
            runId,
            session,
            ...
            action: { id: tc.id, toolName: tc.name, args: tc.input || {} },
          });
          try {
            result = await agentRuntime.dispatchTool(runId, tc.name, tc.input || {}, {
              toolCallId: tc.id,
              providerToolUseId: tc.id,
              scope: { hostId: session.hostId || tc.input?.hostId || 'local' },
              allowApproval: true,
              requestApproval: runtimeDispatchOptions.requestApproval || requestHarnessApproval,
              signal: toolAc.signal,
              onOutput: emitToolDelta,
              onToolCallStarted: emitIdeToolStart,
              executeTool: runtimeDispatchOptions.executeTool,
            });
          } finally {
            unregisterToolCancel();
          }
          ...
          recordIdeAgentSideEffect(runId, tc, result, { ...session, sessionId });

          if (tc.name === 'verify_outcome') {
            recordIdeVerificationOutcome(runId, result, {
              sessionId,
              hostId: session.hostId || tc.input?.hostId || 'local',
              toolUseId: tc.id,
        ...
        if (observationStep.message) toolResults.push({ type: 'text', text: observationStep.message });
        session.messages.push({ role: 'user', content: toolResults });
        if (observationStep.action === 'recover') {
          recordIdeAgentPhase(runId, 'context', 'Runtime recovery required after observing tool results.', { transition: false });
          recordTraceEvent('policy', 'runtime_recovery_required', {
            source: 'ide',
            runId,
            sessionId,
            hostId: session.hostId,
            summary: observationStep.recovery?.reason || 'failed_observation_requires_recovery',
            data: {
              recoveryType: observationStep.recovery?.recoveryType || '',
              failedToolNames: observationStep.recovery?.failedToolNames || [],
            },
          });
        } else {
          recordIdeAgentPhase(runId, 'context', 'Tool results appended; observing outcome and deciding next step.', { transition: false });
        }
      ...
      endIdeAgentRun(runId, { runnerStatus: 'failed', taskStatus: 'blocked', error: `工具调用轮次过多 (${MAX_TOOL_ROUNDS})` });
    } catch (err) {
      if (isAbortError(err)) {
        recordTraceEvent('runtime', 'run_aborted', {
          source: 'ide',
