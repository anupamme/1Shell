...
function normalizeRunSpec(spec = {}) {
  ...
  return {
    ...
    source,
    goal,
    skills: normalizeStringArray(spec.skills),
    skillContext: normalizeObject(spec.skillContext || spec.skill_context),
...
function createInitialAgentState(spec, { runId = null, now = new Date() } = {}) {
  ...
  return {
    ...
    skillContext: normalized.skillContext,
    runnerStatus: 'queued',
    taskStatus: 'unknown',
    currentPhase: '',
    currentTurn: null,
...
function createRuntimeState() {
  return {
    ...
    cognitionHistory: [],
    publicNarrationHistory: [],
    publicNarrationLastFingerprint: '',
    verifier: {
      schemaVersion: 1,
      required: false,
      status: 'not_required',
