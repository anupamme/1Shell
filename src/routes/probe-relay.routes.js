...
function createProbeRelayAdminRouter({ probeRelayService, probeRelayInstallerService }) {
  ...
  router.post('/probe-relay/upstreams/:id/uninstall', async (req, res, next) => {
    try {
      if (!probeRelayInstallerService) return res.status(501).json({ ok: false, error: 'Relay Agent 安装器未启用' });
      const result = await probeRelayInstallerService.uninstallUpstream(req.params.id, {
        clientIp: req.ip,
        deleteConfig: req.body?.deleteConfig === true,
        purgeState: req.body?.purgeState === true,
      });
      res.json({ ok: true, ...result });
    } catch (error) {
      next(error);
  ...
  router.post('/probe-relay/upstreams/:id/migrate-agents', async (req, res, next) => {
    try {
      if (!probeRelayInstallerService) return res.status(501).json({ ok: false, error: 'Relay Agent 安装器未启用' });
      const result = await probeRelayInstallerService.migrateAgentsToUpstream(req.params.id, {
        clientIp: req.ip,
