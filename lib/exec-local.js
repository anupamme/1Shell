...
function execLocalCommand(command, { timeout = 30000, cwd, signal, maxBuffer = 8 * 1024 * 1024, onOutput } = {}) {
...
function spawnLocalScript(command, { timeout = 30000, signal, windowsShell = 'powershell', maxBuffer = 8 * 1024 * 1024, onOutput } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      if (isWindows && windowsShell === 'cmd') {
        child = spawn('cmd.exe', ['/s', '/c', command], { windowsHide: true, env: process.env });
      } else if (isWindows) {
        child = spawn('powershell.exe', ['-NoLogo', '-NoProfile', '-NonInteractive', '-Command', '-'], { windowsHide: true, env: process.env });
      } else {
