...
function encryptText(plainText) {
  ...
  // 安全闸门：未配置 APP_SECRET 时拒绝用公开的回退密钥加密新凭据。
  // 否则 data/1shell.db 一旦泄露，主机密码/私钥可被任何人离线解密。
  // 设置 ONESHELL_ALLOW_FALLBACK_SECRET=1 可显式放行（仅限纯本地、可信环境）。
  if (isUsingFallbackSecret() && process.env.ONESHELL_ALLOW_FALLBACK_SECRET !== '1') {
    const err = new Error(
      '未设置 APP_SECRET，拒绝以默认开发密钥加密凭据。请在 .env 中设置 APP_SECRET 后重试。'
    );
    err.code = 'FALLBACK_SECRET_REFUSED';
    throw err;
  }
