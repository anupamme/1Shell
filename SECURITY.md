...
- **首启免登录**：未配置 `APP_LOGIN_USERNAME/PASSWORD` 时鉴权禁用，方便本地首次使用。但此时服务器**只绑定 `127.0.0.1`（仅本机）**；显式将 `HOST` 设为对外地址却未配置凭据会**拒绝启动**。要对外开放必须先设登录凭据。`start.bat/start.sh` 首次创建 `.env` 时会自动生成 `APP_SECRET`；手动部署未设置时仅告警，不拦截首次添加主机。
