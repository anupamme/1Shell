...
- **首启免登录**：未配置 `APP_LOGIN_USERNAME/PASSWORD` 时鉴权禁用，方便本地首次使用。但此时服务器**只绑定 `127.0.0.1`（仅本机）**；显式将 `HOST` 设为对外地址却未配置凭据会**拒绝启动**。要对外开放必须先设登录凭据。`APP_SECRET` 未设时拒绝加密新凭据（避免用公开默认密钥），启动亦会告警。
