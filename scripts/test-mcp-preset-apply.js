 ...
 *   2) 原生配置写入:applied preset 与 1Shell base 共存
 *   3) ensureNativeConfig 在 apply 之后,本机原生配置真的多出 entry(end-to-end)
 *   4) 空 applied 时只生成 1Shell entry。
 */
...
function makeNativeConfigHarness(tmpDir, mcpPresetStore) {
...
try {
  ...
  // getAppliedRaw 返回原始 token(供原生配置写入用)
  ...
  // ── 2. buildAllMcpEntries + ensureNativeConfig ─────────────────────────
  const nativeConfig = makeNativeConfigHarness(tmpDir, store);

  // codex 原生配置:1shell + github(fetch 已 remove)
  nativeConfig.ensureNativeConfig('codex', { cwd: '/test-cwd' });
  const codexMcpPath = path.join(tmpDir, '.codex', 'mcp.json');
  ...
  // claude-code 原生配置(走 overwrite 'mcp-config'):1shell + memory
  nativeConfig.ensureNativeConfig('claude-code', { cwd: '/test-cwd' });
  const claudeMcpPath = path.join(tmpDir, '.claude', 'mcp-config.json');
  ...
  // ── 3. 空 applied 时只写 1Shell entry ───────────────────────────────
  const emptyStore = createMcpPresetStore({ dataDir: path.join(tmpDir, '_empty') });
  const emptyNativeConfig = makeNativeConfigHarness(path.join(tmpDir, '_empty'), emptyStore);
  emptyNativeConfig.ensureNativeConfig('codex', { cwd: '/empty-cwd' });
  const emptyCodexMcp = JSON.parse(
    fs.readFileSync(path.join(tmpDir, '_empty', '.codex', 'mcp.json'), 'utf8'),
  );
...
console.log('  - 空 applied 时 codex mcp.json 只含 1shell');
