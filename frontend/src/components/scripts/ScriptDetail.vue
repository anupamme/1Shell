    ...
    <div class="shrink-0 px-5 py-3 border-b border-slate-100 dark:border-[#1e293b] flex items-center justify-between gap-3">
      <div class="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div class="min-w-0 flex-1">
          <input
            ...
            type="text"
            placeholder="脚本名称"
            class="min-w-0 w-full text-sm font-bold text-slate-700 dark:text-slate-200 bg-transparent border-b border-transparent hover:border-slate-200 focus:border-blue-400 outline-none"
          />
      ...
      <div class="flex items-center gap-1.5 shrink-0">
        ...
        <button
          v-if="!isNew"
          type="button"
          class="h-8 px-3 rounded-lg bg-gradient-to-r from-blue-500 to-blue-500 text-white text-xs font-semibold shadow-md hover:shadow-lg transition-all"
          @click="emit('run')"
        >▶ 执行</button>
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-5 flex flex-col gap-4">
      ...
      <div class="grid grid-cols-3 gap-3">
        <div class="flex flex-col gap-1">
          ...
          <input
            ...
            class="h-8 px-2 rounded-lg border border-slate-200 dark:border-[#1e293b] bg-slate-50 dark:bg-[#0b1324] text-sm text-slate-700 dark:text-slate-200 text-center outline-none focus:border-blue-400"
          />
        </div>
        <div class="col-span-2 flex flex-col gap-1">
          ...
          <select
            ...
            class="h-8 px-2 rounded-lg border border-slate-200 dark:border-[#1e293b] bg-slate-50 dark:bg-[#0b1324] text-xs text-slate-700 dark:text-slate-200 outline-none focus:border-blue-400"
          >
      ...
      <div class="flex flex-col gap-1">
        ...
        <div class="flex items-center gap-1 flex-wrap p-2 rounded-lg border border-slate-200 dark:border-[#1e293b] bg-slate-50 dark:bg-[#0b1324] min-h-[40px]">
          <span
            ...
            :key="tag"
            class="h-6 px-2 rounded-full border border-blue-200 bg-blue-50 text-blue-600 dark:bg-blue-500/10 dark:border-blue-500/30 dark:text-blue-300 text-[10px] font-medium flex items-center gap-1"
          >
            {{ tag }}
            <button type="button" class="text-blue-400 hover:text-red-500" @click="removeTag(tag)">✕</button>
          </span>
          <input
            v-model="tagInput"
            type="text"
      ...
      <div class="flex flex-col gap-1">
        ...
        <textarea
          ...
          class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-[#1e293b] bg-slate-50 dark:bg-[#0b1324] text-xs text-slate-700 dark:text-slate-200 outline-none resize-none focus:border-blue-400"
        ></textarea>
      </div>
      ...
      <div class="flex flex-col gap-1">
        <div class="flex items-center justify-between">
          ...
          <button type="button" class="text-[10px] text-blue-500 hover:underline font-semibold" @click="addParam">+ 添加参数</button>
        </div>
        <div class="rounded-lg border border-slate-200 dark:border-[#1e293b] divide-y divide-slate-100 dark:divide-[#1e293b] bg-white dark:bg-slate-800 min-h-[44px]">
          ...
          >
            <input
              ...
              type="text"
              placeholder="变量名"
              class="w-32 h-7 px-2 rounded border border-slate-200 dark:border-[#1e293b] bg-slate-50 dark:bg-[#0b1324] text-xs font-mono text-slate-700 dark:text-slate-200 outline-none focus:border-blue-400"
            />
            <select
              v-model="p.type"
            ...
            <label class="flex items-center gap-1 text-[10px] text-slate-500 shrink-0">
              <input v-model="p.required" type="checkbox" class="accent-blue-500" /> 必填
            </label>
            <button
              type="button"
      ...
      <div class="flex flex-col gap-1">
        <div class="flex items-center justify-between">
          ...
          <span class="text-[10px] text-slate-400">
            使用
            <code class="font-mono text-blue-500 dark:text-blue-300 bg-blue-50 dark:bg-blue-500/10 px-1 rounded" v-text="'{{变量名}}'"></code>
            引用参数
          </span>
        </div>
        <textarea
          ...
          class="w-full px-3 py-2 rounded-lg border border-slate-200 dark:border-[#1e293b] bg-slate-900 text-slate-100 font-mono text-xs outline-none resize-y focus:border-blue-400 leading-relaxed"
        ></textarea>
      </div>
    </div>
  </div>
