...
<template>
  <div class="flex-1 flex flex-col min-h-0">
    <div class="shrink-0 px-5 py-3 border-b border-slate-100 dark:border-[#1e293b] flex items-center justify-between">
      ...
      <button
        type="button"
        class="h-7 px-3 rounded-lg border border-slate-200 dark:border-[#1e293b] text-xs text-slate-500 hover:border-blue-300 hover:text-blue-500"
        @click="emit('back')"
      >← 返回脚本列表</button>
    </div>
    ...
    <div class="shrink-0 px-4 py-2 border-t border-slate-100 dark:border-[#1e293b] flex items-center justify-between text-[10px] text-slate-400">
      ...
      <div class="flex items-center gap-2">
        <button
          ...
          class="px-2 py-1 rounded border border-slate-200 dark:border-[#1e293b] hover:border-blue-300 disabled:opacity-50 disabled:cursor-not-allowed"
          @click="gotoPrev"
        >上一页</button>
