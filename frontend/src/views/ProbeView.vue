...
interface RelayBatchResult {
  total: number;
  changed: number;
  skipped: number;
  failed: number;
  results: Array<{ hostId: string; name?: string; result: BridgeExecResult }>;
}
...
async function onInstallAgent(hostId: string): Promise<void> {
  ...
  if (!relay) {
    const proceed = window.confirm('当前未选择 Relay 上报入口。若这台 1Shell 没有公网可访问地址，Agent 即使安装成功也无法回传数据。建议先在“中继设置”安装/选择 Relay；仍要继续直连安装吗？');
    if (!proceed) {
  ...
  agentActionDialog.value = {
    ...
    hostId,
    result: {
      stdout: relay ? '正在通过 Relay 下载、注册并启动 1Shell Probe Agent...' : '正在直连安装 1Shell Probe Agent；若当前 1Shell 无公网入口，安装后将无法回传数据。',
      stderr: '',
      exitCode: -1,
      durationMs: 0,
    },
...
function editRelay(upstream: RelayUpstream): void {
  ...
  relayForm.relayPort = upstream.relayPort || '3302';
  relayForm.syncToken = '';
...
async function deleteRelayUpstream(id: string): Promise<void> {
  if (!window.confirm('只删除本机保存的 Relay 上游配置，不会停止 VPS 上的 Relay Agent，也不会卸载其它 VPS 上的探针 Agent。确认继续？')) return;
  relayLoading.value = true;
  try {
    ...
    notify.success('Relay 本地配置已删除', 2500);
    await loadRelayUpstreams();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
...
async function uninstallRelayUpstream(id: string): Promise<void> {
  if (!window.confirm('即将在 Relay VPS 上停止服务并删除 /opt/1shell/probe-relay 与配置文件。确认执行远端卸载？')) return;
  const purgeState = window.confirm('是否同时清空 Relay VPS 上的历史状态？清空后旧探针 token 会失效，需要重新安装或重新注册探针 Agent。');
  const deleteConfig = window.confirm('卸载远端 Relay Agent 后，是否同时删除本机保存的 Relay 上游配置？');
  relayLoading.value = true;
  try {
    const response = await requestJson<{ ok: boolean; result: BridgeExecResult; deleted: boolean; purged: boolean }>(`/api/probe-relay/upstreams/${encodeURIComponent(id)}/uninstall`, {
      method: 'POST',
      body: JSON.stringify({ deleteConfig, purgeState }),
    });
    if (response.result.exitCode !== 0) {
      notify.error(`远端 Relay 卸载失败：${response.result.stderr || response.result.stdout || '远端命令失败'}`, 7000);
      return;
    }
    if (response.deleted && selectedRelayUpstreamId.value === id) selectRelayForInstall('');
    notify.success(response.purged ? '远端 Relay 已卸载，历史状态已清空' : '远端 Relay 已卸载', 4000);
    await loadRelayUpstreams();
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    notify.error(`远端 Relay 卸载失败：${msg}`, 7000);
  } finally {
    relayLoading.value = false;
  }
}

async function migrateRelayAgents(id: string): Promise<void> {
  if (!window.confirm('将扫描所有已纳管 VPS：发现 /etc/1shell-probe-agent.env 后，把 SERVER_URL 改成该 Relay 地址并重启探针 Agent。确认继续？')) return;
  relayLoading.value = true;
  try {
    const response = await requestJson<{ ok: boolean } & RelayBatchResult>(`/api/probe-relay/upstreams/${encodeURIComponent(id)}/migrate-agents`, {
      method: 'POST',
      body: JSON.stringify({}),
    });
    const failedHosts = response.results.filter((item) => item.result.exitCode !== 0).slice(0, 3).map((item) => item.name || item.hostId);
    if (response.failed > 0) {
      notify.warn(`迁移完成但有失败：已改 ${response.changed} 台，跳过 ${response.skipped} 台，失败 ${response.failed} 台（${failedHosts.join('、')}）`, 8000);
    } else {
      notify.success(`探针入口迁移完成：已改 ${response.changed} 台，跳过 ${response.skipped} 台`, 5000);
    }
    await loadRelayUpstreams();
    await loadProbes(true);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    notify.error(`迁移探针入口失败：${msg}`, 8000);
  } finally {
    relayLoading.value = false;
  }
}
  ...
  try {
    const response = await requestJson<{ ok: boolean; upstream: RelayUpstream | null; result: BridgeExecResult }>('/api/probe-relay/upstreams/install', {
      ...
      body: JSON.stringify({
        ...
        relayPort: Number(relayForm.relayPort) || 3302,
            ...
            <div class="space-y-3">
              ...
              >
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                  ...
                  <button type="button" class="text-[11px] px-2 py-0.5 rounded border border-amber-200 dark:border-amber-700 text-amber-600 dark:text-amber-300 disabled:opacity-50" :disabled="relayLoading || !upstream.enabled || !upstream.relayHostId" @click="migrateRelayAgents(upstream.id)">迁移探针入口</button>
                  ...
                  <button type="button" class="text-[11px] px-2 py-0.5 rounded border border-red-200 dark:border-red-700 text-red-600 dark:text-red-300 disabled:opacity-50" :disabled="relayLoading" @click="deleteRelayUpstream(upstream.id)">删本地</button>
                  <button type="button" class="text-[11px] px-2 py-0.5 rounded border border-red-300 dark:border-red-700 text-red-700 dark:text-red-300 disabled:opacity-50" :disabled="relayLoading || !upstream.relayHostId" @click="uninstallRelayUpstream(upstream.id)">卸载远端</button>
                </div>
              </div>
            </div>
            <div class="space-y-3">
              ...
              <div class="grid grid-cols-3 gap-2">
                <input v-model="relayForm.relayPort" class="h-9 px-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-950 text-xs text-slate-700 dark:text-slate-200" placeholder="3302" />
                <div class="col-span-2 h-9 px-3 rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-950/60 text-xs text-slate-500 dark:text-slate-400 flex items-center truncate">
                  {{ selectedRelayHost ? `http://${selectedRelayHost.host}:${relayForm.relayPort || 3302}` : '选择 VPS 后自动生成 Relay 地址' }}
                </div>
              </div>
